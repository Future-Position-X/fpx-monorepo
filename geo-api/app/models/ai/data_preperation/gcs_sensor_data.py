...
class SensorData:
    ...
    def __get_token(self):
        r = requests.get(self.request_url+"/Login/" + self.user_id + "?pw=" + self.pwd, headers=self.request_headers)
        return json.loads(r.text)
    
    @classmethod
    def __get_sensors_index(self, token):
        r = requests.get(self.request_url+"/ListCounter/" + self.user_id + "?token=" + token, headers=self.request_headers)
        return json.loads(r.text)
    ...
    def __get_sensor_data(self, token, cid, start_date, end_date):
        ...
        r = requests.get(self.request_url+"/CountData/" + self.user_id + "", params=payload, headers=self.request_headers)
        return json.loads(r.text)
    ...
    def get_data(self, start_date, end_date):
        ...
        with concurrent.futures.ThreadPoolExecutor() as executor:  
            for sensor_data in executor.map(lambda s: self.__get_sensor_data(token, s, start_date, end_date), self.sensors_in_use):
                ...
                df_main = pd.merge(df_main, df[['s' + cid + '_in', 's' + cid + '_out']], left_on='cDte', right_on='cDte', how='left')
