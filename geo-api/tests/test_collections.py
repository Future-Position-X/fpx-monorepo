...
def test_api_can_get_collection_by_uuid(client, collection):
    result = client.get("/collections/{}".format(collection["uuid"]))
...
def test_collection_can_be_copied_to_new_collection(
    ...
    res = client.post("/collections/{}/copy".format(collection["uuid"]))
    assert res.status_code == 201
    collection_hash = json.loads(res.data.decode("utf-8"))

    result = client.get("/collections/{}/items".format(collection_hash["uuid"]))
    assert result.status_code == 200
...
def test_collection_can_be_copied_to_other_collection(
    ...
    res = client.post(
        "/collections/{}/copy/{}".format(collection["uuid"], collection_empty["uuid"]),
        data=None,
    )
    assert res.status_code == 201
    collection_hash = json.loads(res.data.decode("utf-8"))
    ...
    result = client.get("/collections/{}/items".format(collection_empty["uuid"]))
    assert result.status_code == 200
    assert item["properties"]["name"] in str(result.data)
    assert str(item["uuid"]) not in str(result.data)
